...
当前档案 `AG162` 映射代理商“沈壮志”的租户、包名、穿山甲 site/content App、Taku App/激励广告位以及
唯一产物名。后续代理商按 `android-djx-runtime/profiles/README.md` 新增同名档案。

`AG162` 当前 `profileVersion` 为 3，绑定 Taku 6.6.30 与 Pangle/GroMore 7.6.1.1。档案只声明 SDK 版本和公开 App/placement 身份，不声明 `networkFirmId`、adsource 或供应商选择；实际广告源由 Taku 服务端策略决定。
...
8. 官方 13 AAR、全部嵌套 jar/DEX、官方 `keep.xml`、合并组件/资源/native library 和唯一 `TTAdSdk` 与 lock 一致，且危险权限未进入合并 manifest。

项目的复用入口为：

```bash
SKIT_REUSABLE_PACKAGE_GATE=/absolute/path/to/verify-agent-apk.sh \
SKIT_PROFILE_CODE=<PROFILE_CODE> \
APK_FILE=/absolute/path/to/app.apk \
./android-djx-runtime/run-reusable-package-gate.sh
```

个人 gate 若需要反向调用项目专属检查，应在解析并验证 `--project/--profile/--apk` 后执行：

```bash
SKIT_PROFILE_CODE="$PROFILE_CODE" APK_FILE="$APK_FILE" \
  "$PROJECT_DIR/android-djx-runtime/verify-production-apk.sh"
```

正式交付同时运行项目 verifier 与复用 package gate；两者都必须消费相同 bundle lock 和 profile SDK 版本，任一失败都阻止交付。
